...
def DepF(flag00,flag11,list3,k,Dict,string):
	print list3[k-1],"has RAW dependency with",i
        flag00[r].append(k-1)
        flag11[r].append(list3[k-1])
        if list3[k-1] not in Dict1[string]:
        	Dict1[string].append(list3[k-1])
...
def bdeep(lw1,lw2,k):
	for i in lis3[k:]:
		r=lis3.index(i)
		if i[0]=='add' or i[0]=='mul' or i[0]=='sub' or i[0]=='addi' or i[0]=='div' or i[0]=='addiu' or i[0]=='addu':
			if lw1==chardel(i[1]):
				print lis3[k-1],"has WAR dependency with",i
				flag[r].append(k-1)
				flag1[r].append(lis3[k-1])
...
for i in range(len(lis3)):
	for j in range(len(lis3[i])):
		try:
			...
			if lis3[i][j]=='li' and lis3[i][j+1]!='$v0':
			...
			elif lis3[i][j]=='add' or lis3[i][j]=='mul' or lis3[i][j]=='div' or lis3[i][j]=='sub' or lis3[i][j]=='addi' or lis[i][j]=='addiu':
				add1=lis3[i][j+1]
				add2=lis3[i][j+2]
				add3=lis3[i][j+3]
				k1=chardel(add1)
...
for i in range(0,len(ins)):
	print ins[i]
	if ins[i][0]=='addi' or ins[i][0]=='add' or ins[i][0]=='mul' or ins[i][0]=='sub' or ins[i][0]=='div' or ins[i][0]=='addu' or ins[i][0]=='addiu':
		try:
			...
			if ins[i+1][0]=='blez' or ins[i+1][0]=='bltz' or ins[i+1][0]=='bgtz' or ins[i+1][0]=='bgez':
				...
				if ins[i+1][1]==ins[i][1]:
					...
					time1+=1
					time.append(time1)
			if ins[i+1][0]=='addi' or ins[i+1][0]=='add' or ins[i+1][0]=='mul' or ins[i+1][0]=='sub' or ins[i+1][0]=='div' or ins[i+1][0]=='addiu' or ins[i+1][0]=='addu':
				ins[i+1][1]=chardel(ins[i+1][1])
				ins[i+1][2]=chardel(ins[i+1][2])
				...
				if ins[i][1]== ins[i+1][2] or ins[i+1][3]==ins[i][1] or ins[i][1]==ins[i+1][1] or ins[i+1][1]==ins[i][2] or ins[i+1][1]==ins[i][3]:
					...
					time1+=1
					time.append(time1)
			elif ins[i+1][0]=='lw' or ins[i+1][0]=='lb':
				ins[i+1][1]=chardel(ins[i+1][1])
				ins[i+1][2]=chardel(ins[i+1][2])
		...
		try:
			...
			if ins[i+1][0]=='blez' or ins[i+1][0]=='bltz' or ins[i+1][0]=='bgtz' or ins[i+1][0]=='bgez':
				...
				if ins[i][1]==ins[i+1][2] or ins[i][2]==ins[i+1][2]:
					...
					time1=time1+1
					time.append(time1)
			elif ins[i+1][0]=='lw' or ins[i+1][0]=='move' or ins[i+1][0]=='li' or ins[i+1][0]=='lb':
				ins[i+1][1]=chardel(ins[i+1][1])
		...
		try:
			...
			if ins[i+1][0]=='addi' or ins[i+1][0]=='add' or ins[i+1][0]=='mul' or ins[i+1][0]=='sub' or ins[i+1][0]=='div' or ins[i+1][0]=='addiu' or ins[i+1][0]=='addu':
				...
				if ins[i][2]==ins[i+1][1] or ins[i][2]==ins[i+1][2] or ins[i][2]==ins[i+1][3] or ins[i][1]==ins[i+1][1] :
					...
					time1=time1+1
					time.append(time1)
			elif ins[i+1][0]=='sw' or ins[i+1]=='sb':
				ins[i+1][1]=chardel(ins[i+1][1])
				ins[i+1][2]=chardel(ins[i+1][2])
...
def func(l5,j,time):
	...
	for k in range(len(l5)):
		if j==k:
			...
			print lu,"List"
	for n in lu:
		if ins[n][0]=='lw' or ins[n][0]=='sw' or ins[n][0]=='lb' or ins[n][0]=='sb':
			if maxi<time[n]:
				par=n
				maxi=time[n]
				ti=2
